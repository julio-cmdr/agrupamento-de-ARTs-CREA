...
int group_is_valid(uint64_t *group, int group_size, Project *projects, int number_of_projects){
    uint8_t bits_sum;
    ...
    for (int j = number_of_projects-1; j >= 0; j--){
        ...
        sum_last_combination += projects[number_of_projects-1-j].art * (((group[group_size-1]) >> j) & 1);
...
int parallel_analysis(uint64_t *group, int group_size, Project *projects, int number_of_projects, uint64_t start, uint64_t number_of_combinations){
    ...
    for(uint64_t i = start; i < number_of_combinations; i++){
        group[group_size-1] = i;
        
        if(group_is_valid(group, group_size, projects, number_of_projects))
            return 1;
    }
    return 0;
}
